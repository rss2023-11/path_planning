...
class PathPlan(object):
    ...
    def map_cb(self, msg):
        ...
        # Dilate + Erode the obstacles in the map
        only_eroded_map = ndimage.binary_erosion(map2d, se_erode)
        only_dilated_map = ndimage.binary_dilation(map2d, se_dilate)
        # erode_dilate_map = ndimage.binary_dilation(only_eroded_map, se_dilate)
        dilate_erode_map = ndimage.binary_erosion(only_dilated_map,se_erode)
        rospy.loginfo(dilate_erode_map)

        # Create the OccupancyGrid message and fill in its fields
        map_msg = OccupancyGrid()
        map_msg.header.frame_id = 'map'
        map_msg.info.resolution = 1.0
        map_msg.info.width = dilate_erode_map.shape[1]
        map_msg.info.height = dilate_erode_map.shape[0]
        ...
        map_msg.data = list(dilate_erode_map.ravel())

        self.map_pub.publish(map_msg)

        self.map = dilate_erode_map
        ...
        # axs[1].imshow(only_dilated_map, cmap='gray')
        # axs[1].set_title('Dilated Map')
        # axs[2].imshow(only_eroded_map, cmap='gray')
        # axs[2].set_title('Eroded Map')
        # axs[3].imshow(dilate_erode_map, cmap='gray')
        # axs[3].set_title('Dilated+Eroded Map')
        # plt.show()
